...
export function Products() {
  ...
  const suggestions = typed ? [
    ...categories.map(item => ({ label: item.name, category: item.slug })),
    ...catalogProducts.map(product => ({ label: product.name, q: product.name })),
    ...catalogProducts.flatMap(product => [product.grade, product.cas].filter(Boolean).map(label => ({ label, q: label })))
  ].filter((item, index, items) => item.label.toLowerCase().includes(typed) && items.findIndex(candidate => candidate.label.toLowerCase() === item.label.toLowerCase()) === index).slice(0, 7) : [];
  const applySuggestion = suggestion => {
    setDraft(suggestion.q || '');
    update(suggestion.category ? { category: suggestion.category, q: '' } : { q: suggestion.q });
  };
  const searchControls = <><form className="search-bar product-search-bar" onSubmit={event => { event.preventDefault(); update({ q: draft.trim() }); }} role="search"><Search size={19} aria-hidden="true" /><div className="product-search-input"><input value={draft} onChange={event => setDraft(event.target.value)} placeholder="Search products..." aria-label="Search products" autoComplete="off" />{suggestions.length > 0 && <div className="product-search-suggestions" role="listbox" aria-label="Search suggestions">{suggestions.map(suggestion => <button key={`${suggestion.category || 'query'}-${suggestion.label}`} type="button" className="suggestion-item" onClick={() => applySuggestion(suggestion)}>{suggestion.label}</button>)}</div>}</div><button className="button" type="submit">Search <Arrow size={16} /></button></form><label className="mobile-category-select">Category<select value={category} onChange={event => update({ category: event.target.value })}><option value="">All products</option>{categories.map(item => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label></>;
  return <><PageIntro label="OUR CHEMICAL PORTFOLIO" title={<>Materials for<br /><em>what comes next.</em></>}>Explore our product list. Find a material, review the available details, and send your requirements to our team.</PageIntro>{showMobileControls && <div className="mobile-catalog-sticky">{searchControls}</div>}<section className="catalog-section section"><div className={`catalog-search-controls ${showMobileControls ? 'scrolled' : ''}`}>{searchControls}</div><div className="catalog-layout"><aside className="filters"><span className="eyebrow">CATEGORIES</span><button className={!category ? 'selected' : ''} onClick={() => update({
            category: ''
...
export function Contact() {
  ...
  async function submit(e) {
    ...
    setError('');
    try {
      const productName = (catalog.data?.products || catalogProducts).find(p => p.slug === form.product)?.name || form.product || 'Not specified';
      ...
      window.open(`https://wa.me/${company.whatsapp || '923245121105'}?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer');
