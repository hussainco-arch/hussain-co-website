...
export function ProductCard({
  ...
  return <article className="product-card"><Link className={`product-image ${product.category}`} to={`/products/${product.slug}`} tabIndex={-1} aria-hidden="true"><img src={`${product.image}?v=2`} alt="" loading="lazy" /><span className="product-category">{category?.short || product.category}</span><span className="stock-caption">Illustrative image</span><span className="product-arrow"><Arrow size={22} /></span></Link><div className="product-copy"><span className="product-meta">{product.origin === 'Confirm with quotation' ? 'SPECIFICATION ON REQUEST' : product.origin}</span><h3><Link to={`/products/${product.slug}`}>{product.name}</Link></h3><p>{product.packaging} <span> / </span> {product.grade}</p><Link className="product-quote" to={`/contact?product=${product.slug}`}>Request a quote <Arrow size={16} /></Link></div></article>;
}
