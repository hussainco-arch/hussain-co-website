...
const AnimatedFeatureCard = React.forwardRef(({ className, index, tag, title, imageSrc, color, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-20 rounded-lg border bg-background/80 p-4 backdrop-blur-sm dark:bg-background/60" style={{ background: 'rgba(255, 255, 255, 0.8)', borderColor: 'rgba(0,0,0,0.1)' }}>
        <span
        ...
        <p className="text-base text-card-foreground" style={{ color: '#000' }}>{title}</p>
      </div>
    </motion.div>
  );
});
